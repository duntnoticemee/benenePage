...
const App = () => {
  ...
  const topZRef = useRef(1000); //use ref wont cause re render when change
...
const [mainInitial, setMainInitial] = useState(() => ({
  x: Math.round(window.innerWidth / 2 - 800 / 2), // width of main card 
  y: Math.round(window.innerHeight / 2 - 500 / 2), // height of main card
}));
...
useEffect(() => {
  ...
  const handleOpenCard = (cardData) => {
    setOpenCards((prev) => {
      ...
      // assign z-index
      topZRef.current += 1; //ref does not cause re-render when change
  ...
  // keep main centered on resize using CSS flex container, no resize listener needed
  return (
    <div
      style={{
        height: '100vh',
      ...
      onWheel={(e) => {
        // allow background wheel only if no popup content prevents it; otherwise stop
        // prevents accidental page scroll in some browsers, but if  prefer to keep page scroll,
        // remove this handler.
      }}
    >
      {/* top left decorative star, sun, moon */}
      <img
        //star 
  ...
  >
          <div> 
            ...
            {/* lorem ipsum for scroll testing */}
